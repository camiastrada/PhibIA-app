...
function Home() {
  ...
  return (
    <div className="flex flex-col justify-around items-center w-9/10 h-4/5 md:h-7/10 md:w-4/5 bg-white/85 border-2 border-white backdrop-blur-xs rounded-3xl">
      <div id="titleSection">
      ...
      <div id="buttonsSection" className="flex flex-col justify-around items-center">
        <button type="button" onClick={() => {setListening(!listening); setIsImport(false)}} className={`flex flex-row justify-center cursor-pointer ${listening? "bg-transparent text-red-600 animate-pulse" : "bg-[#43A047] rounded-xl shadow-lg hover:shadow-xl hover:bg-[#357a38] text-white w-60 px-6 py-3 text-lg font-semibold"}`}>
          {listening ? (
            <>
        ...
        <button type="button" onClick={() => {setIsImport(true); setListening(false)}} className={`mt-4 text-[#004D40] hover:text-[#02372E] flex-row justify-center items-center ${listening ? "hidden" : "flex"}`}>
          Importar grabación
